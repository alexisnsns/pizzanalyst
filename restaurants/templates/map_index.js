mapboxgl.accessToken = document
  .getElementById("map")
  .getAttribute("data-mapbox-access-token");

var restaurantsAddresses = document
  .getElementById("map")
  .getAttribute("data-restaurants-addresses")
  .split(",");

var restaurantsNames = document
  .getElementById("map")
  .getAttribute("data-restaurants-names")
  .slice(2, -2)
  .split(",");
restaurantsNames = restaurantsNames.map((item) =>
  item.trim().replace(/'/g, "")
);

var restaurantsIndexes = document
  .getElementById("map")
  .getAttribute("data-restaurants-indexes")
  .slice(1, -1)
  .split(",");
restaurantsIndexes = restaurantsIndexes.map((item) =>
  item.trim().replace(/'/g, "")
);

var faviconUrl = document
  .getElementById("map")
  .getAttribute("data-favicon-url");
