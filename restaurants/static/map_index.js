...
// get addresses and split from comma
var list = (document.getElementById('map').getAttribute('data-restaurants-addresses')).split(',');
...
const fetchPromises = list.map(address =>
  ...
  .then(dataArray => {
    dataArray.forEach((data, index) => {
      ...
      var marker = new mapboxgl.Marker(customPin)
        ...
        .setPopup(new mapboxgl.Popup().setHTML('<h3>' + restaurantsNames[index] + '</h3>' ))
        .addTo(map);
